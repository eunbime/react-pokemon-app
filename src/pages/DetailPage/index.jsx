...
const DetailPage = () => {
  ...
  async function fetchPokemonData(id) {
    ...
    function filterAndFormatDescription(flavorText) {
    ...
    function formatPokemonAbilities(abilities) {
      return abilities
        .filter((_, index) => index <= 1) // index가 1 이하일 때
        .map((obj) => obj.ability.name.replaceAll("-", " "));
    }
  }

  async function getNextAndPreviousPokemon(id) {
    const urlPokemon = `${baseUrl}?limit=1&offset=${id - 1}`; // limit 1이기 때문에 offset -1 해준다.
    const { data: pokemonData } = await axios.get(urlPokemon);

    const nextResponse =
      pokemonData.next && (await axios.get(pokemonData.next));
    const previousResponse =
      pokemonData.next && (await axios.get(pokemonData.previous));

    return {
      next: nextResponse?.data?.results?.[0]?.name, // 옵셔널하게 방어코드
