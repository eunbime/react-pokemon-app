...
const DamageRelations = ({ damages }) => {
  const [damagePokemonForm, setDamagePokemon] = useState();
  ...
  const filterForUniqueValues = (valueForFiltering, damageValue) => {
    return valueForFiltering.reduce((acc, currentValue) => {
      ...
      const filterACC = acc.filter((a) => a.name !== name); // 같은 값을 없애줌
  ...
  const joinObjects = (props, string) => {
    ...
    const result = Object.entries(secondArrayValue).reduce(
      (acc, [keyName, value]) => {
        const result = firstArrayValue[keyName].concat(value); // 들어온 값을 쌓는다
  ...
  const filterDamageRelations = (valueFilter, damage) => {
    const result = Object.entries(damage)
      ...
      .reduce((acc, [keyName, value]) => {
        const keyWithValueFilterRemove = keyName.replace(valueFilter, ""); // _from, _to 없애기
